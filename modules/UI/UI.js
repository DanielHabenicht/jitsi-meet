...
UI.initEtherpad = (name) => {
    config.etherpad_base = "https://etherpad.test.jmserv.de/p/";
 ...
 */
UI.initGenericIFrame = (name) => {
    debugger;
    config.genericIFrameTemplateUrl = "https://excalidraw.test.jmserv.de/";

    if (genericIFrameManager || !config.genericIFrameTemplateUrl || !name) {
        return;
    }
    ...
    const url = new URL(name, config.genericIFrameTemplateUrl);

    APP.store.dispatch(setGenericIFrameUrl(url.toString()));

    if (config.openSharedDocumentOnJoin) {
        genericIFrameManager.toggleGenericIFrame();
    }
};
